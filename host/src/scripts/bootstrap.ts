...
async function main() {
  ...
  try {
    ...
    // ── 6c. Namespace rules ───────────────────────────────────────────────
    // Seeded by V81, not here. They used to be seeded in this file, which meant
    // an existing installation never got them: update.sh runs bootstrap with
    // --skills-only so a tuned configuration is not overwritten, and the rules
    // rode along in the part that is deliberately skipped. A migration runs on
    // both paths, so that is where they live now — with ON CONFLICT DO NOTHING,
    // which keeps the additive behaviour this code already had.
