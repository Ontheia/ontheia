...
| Test Model ID | Text | HTTP only | Model ID for POST connection tests (e.g. `gpt-5.6-terra`). |
...
| Model ID | Text | Exact model identifier as used by the API (e.g. `gpt-5.6-terra`). |
