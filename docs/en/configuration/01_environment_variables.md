...
| `EMBEDDING_CONFIG_PATH` | Path to the embedding configuration file (`embedding.config.json`). **Optional:** The database configuration (Admin → Settings → Embedding) takes precedence. This file is only used as a fallback. |
