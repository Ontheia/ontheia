...
*   **Parameters:**
    ...
    *   `chain_id`: (Optional) Name or UUID of a specific chain that runs bound to the agent — instead of the agent's default chain. Without `task_id` and `chain_id`, the default chain runs if one exists, otherwise an LLM call. (See [Agent-to-Chain Binding](/en/admin/chains/06_agent_chain_binding/).)
