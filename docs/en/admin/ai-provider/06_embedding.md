...
2. **Add an embedding model** to the provider with:
   ...
   - **Metadata:** `dimension`, `metric`, `normalize`, and optionally `endpoint` (see [Manage Models](/en/admin/ai-provider/03_models/))
...
If no embedding configuration is stored in the database, Ontheia falls back to `embedding.config.json`. The path can be overridden via the environment variable `EMBEDDING_CONFIG_PATH`. See [Environment Variables](/en/configuration/01_environment_variables/).
