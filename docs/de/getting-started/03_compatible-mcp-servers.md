...
| **pdfnative-mcp** | PDFs erstellen, signieren (PAdES), prüfen und umwandeln — PDF/A, Formulare, Barcodes | [github.com/Nizoka/pdfnative-mcp](https://github.com/Nizoka/pdfnative-mcp) |
...
<details>
<summary>⚙ pdfnative-mcp — Konfiguration</summary>
...
```json
{
  "mcpServers": {
    "pdfnative": {
      ...
      "env": {
        "PDFNATIVE_MCP_OUTPUT_DIR": "/pfad/zur/ausgabe"
      }
    }
  }
}
...
Benötigt Node.js ≥ 22. `PDFNATIVE_MCP_OUTPUT_DIR` legt fest, wohin erzeugte PDFs geschrieben werden. Keine API-Keys nötig — läuft vollständig lokal.
...
| **duckduckgo-mcp-server** | Websuche via DuckDuckGo (ohne API-Key) | [github.com/nickclyde/duckduckgo-mcp-server](https://github.com/nickclyde/duckduckgo-mcp-server) |
...
<details>
<summary>⚙ duckduckgo-mcp-server — Konfiguration</summary>
...
> DuckDuckGo benötigt keinen API-Key — läuft ohne weitere Konfiguration.
...
| **jellyfin-mcp** | Jellyfin-Mediaserver steuern — Bibliothek, Wiedergabe, Playlisten | [github.com/jaredtrent/jellyfin-mcp](https://github.com/jaredtrent/jellyfin-mcp) |
...
<details>
<summary>⚙ jellyfin-mcp — Konfiguration</summary>
...
> API-Key im Jellyfin-Dashboard unter *Verwaltung → API-Keys* anlegen. `--read-only` beschränkt auf Lesezugriff, `--disable-destructive` blockiert Löschen/Neustart/Herunterfahren.
