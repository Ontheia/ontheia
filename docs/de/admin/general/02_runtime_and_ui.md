...
> **Warum nur Anthropic?** Anthropic ist der Anbieter, bei dem Caching *ohne Gegenwehr* teurer sein kann als die Ersparnis: Das Schreiben des Caches (`cache_creation`) wird mit ~1,25× des normalen Input-Preises berechnet. Wird der gecachte Prefix **nicht innerhalb der 5-Minuten-TTL wiedergelesen** — etwa bei sporadischen Einzelläufen (Cron-Tasks mit einem einzigen LLM-Aufruf, lange Denkpausen im Chat) — zahlst du den Write-Aufschlag, ohne je den günstigen Read zu kassieren: **+25 % statt Ersparnis**. Bei Anthropic entscheiden *wir*, ob `cache_control` gesetzt wird — deshalb funktioniert ein An/Aus-Schalter hier technisch.
>
> **OpenAI (außer gpt-5.6), xAI und Mistral** cachen automatisch und **ohne Write-Aufschlag** (nur „Input" und „Cached input", keine Write-Spalte). Dort kann Caching nie teurer sein als kein Caching — dieser Schalter betrifft sie nicht.
>
> **Ausnahme seit gpt-5.6 (Juli 2026):** Die gpt-5.6-Modelle (`sol`/`terra`/`luna`) berechnen jetzt ebenfalls einen Cache-Write-Aufschlag von **1,25×** — exakt dieselbe Mechanik wie bei Anthropic (Preistabelle: z. B. `gpt-5.6-sol` $5,00 Input → $6,25 Cache-Write). Anders als bei Anthropic ist dieses Caching bei OpenAI aber *implizit* (Modus `auto`, laut OpenAI-Dokumentation der Default) — Ontheia sendet dafür keinen eigenen Parameter, den dieser Schalter ein- oder ausschalten könnte. **Dieser Schalter deckt gpt-5.6 also nicht ab.** OpenAI bietet stattdessen einen `explicit`-Modus, mit dem gezielt markiert wird, welche Prefixe gecacht werden — das ist keine einfache An/Aus-Einstellung, sondern eine strukturelle Änderung am Request-Aufbau, und aktuell nicht implementiert (siehe Roadmap: Responses-API-Ausbau).
...
**Bei gpt-5.6 beobachten statt abschalten:** Die ⚡-Badge im Trace-Panel zeigt seit Kurzem auch OpenAIs Cache-Write-Tokens an (als Cache-Creation, analog zu Anthropic). Damit lässt sich beurteilen, ob gpt-5.6 in eurem Nutzungsmuster tatsächlich Mehrkosten verursacht (sporadische Läufe) oder ob die Ersparnis durch wiederholte Reads überwiegt (dichte Tool-Schleifen, schnelle Chat-Folgen).
