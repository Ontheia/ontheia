...
2. **Embedding-Modell hinzufügen** mit:
   ...
   - **Metadaten:** `dimension`, `metric`, `normalize` und optional `endpoint` (siehe [Modelle verwalten](/de/admin/ai-provider/03_models/))
...
Ist keine Embedding-Konfiguration in der Datenbank gespeichert, fällt Ontheia auf `embedding.config.json` zurück. Der Pfad kann über die Umgebungsvariable `EMBEDDING_CONFIG_PATH` überschrieben werden. Siehe [Umgebungsvariablen](/de/configuration/01_environment_variables/).
